...
import { deepMerge, isArray, isEntityLink, isObject, makeFieldKey, resolveEntityLink, transformArguments } from './utils';
    ...
    for (const selection of selections) {
      if (selection.kind === 'TypenameField') {
        ...
        if (v === null || v === undefined) {
          ...
          deepMerge(current[selection.alias ?? selection.name] as Record<string, unknown>, denormalizeObject(v, selection.children));
        } else if (isArray(v)) {
          current[selection.alias ?? selection.name] ??= [];
          for (const [i, element] of v.entries()) {
            if (element === null) {
              ...
              (current[selection.alias ?? selection.name] as unknown[])[i] = denormalizeObject(element as Storage, selection.children);
            }
          }
        }

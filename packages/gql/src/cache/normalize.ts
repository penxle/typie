...
export const normalize = (storeSchema: StoreSchema, variables: Variables, data: Data) => {
  ...
  const normalizeObject = (value: Data, selections: Selection[]) => {
    ...
    for (const selection of selections) {
      if (selection.kind === 'TypenameField') {
        ...
        if (v === null) {
          ...
          current[key] = v.map((element) => (element === null ? null : normalizeObject(element as Data, selection.children)));
        } else if (isObject(v)) {
    ...
    if (isEntityLink(current)) {
      ...
      storage[entityKey] ??= {};
      deepMerge(storage[entityKey], Object.fromEntries(Object.entries(current).filter(([key]) => key !== entityLinkKey)));

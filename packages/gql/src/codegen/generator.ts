...
export const buildSelectionsTSType = (
  ...
  if (parentType) {
    if (parentType.kind === 'Interface') {
      return AST.b.tsUnionType(parentType.implementations.map((v) => buildSelectionsTSType(selections, { kind: 'Object', name: v })));
    } else if (parentType.kind === 'Union') {
      return AST.b.tsUnionType(parentType.members.map((v) => buildSelectionsTSType(selections, { kind: 'Object', name: v })));
    }
  }
    ...
    if (selection.kind === 'TypenameField') {
      ...
      literal.members.push(
        AST.b.tsPropertySignature.from({
          ...
          typeAnnotation: AST.b.tsTypeAnnotation(
            wrapTSType(buildScalarRefTSType(selection.type.name, 'Output'), selection.type.isList, selection.type.isNonNull),
          ),
        }),
      );
      ...
      literal.members.push(
        AST.b.tsPropertySignature.from({
          ...
          optional: !selection.type.isNonNull,
          typeAnnotation: AST.b.tsTypeAnnotation(
            wrapTSType(buildSelectionsTSType(selection.children, selection.type), selection.type.isList, selection.type.isNonNull),
          ),
        }),
      );
    ...
    if (variable.kind === 'ScalarField') {
      base.members.push(
        AST.b.tsPropertySignature.from({
          ...
          typeAnnotation: AST.b.tsTypeAnnotation(
            wrapTSType(buildScalarRefTSType(variable.type.name, 'Input'), variable.type.isList, variable.type.isNonNull),
          ),
        }),
      );
...
export const buildGraphQLFunctions = (artifacts: Artifact[]): AST.Program => {
  const program = AST.b.program([
    ...
    ...artifacts.map((artifact) =>
      AST.b.importDeclaration.from({
        importKind: 'type',
        source: AST.b.stringLiteral(`../artifacts/${artifact.kind === 'fragment' ? 'fragments' : 'operations'}/${artifact.name}`),
        ...
        AST.b.exportNamedDeclaration(
          AST.b.tsDeclareFunction.from({
            ...
            params: [
              AST.b.identifier.from({
                name: 'prop',
                typeAnnotation: AST.b.tsTypeAnnotation(AST.b.tsTypeReference(AST.b.identifier(`types.${artifact.name}`))),
              }),
              AST.b.identifier.from({
                name: 'doc',
                typeAnnotation: AST.b.tsTypeAnnotation(AST.b.tsTypeReference(AST.b.identifier(`types.${artifact.name}`))),

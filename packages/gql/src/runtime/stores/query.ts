...
type StoreOutput<T extends $StoreSchema<Kind>> = T['$output'] | (T['$meta'] extends { mode: 'manual' } ? undefined : never);
    ...
    return async (
      ...
      return Object.assign(store, {
        refetch: async (newVariablesOrContext?: T['$input'] | RequestContext, context?: RequestContext) => {
          const effectiveVariables = schema.meta.inputless === 'true' ? {} : ((newVariablesOrContext as T['$input']) ?? variables ?? {});
          const effectiveContext = schema.meta.inputless === 'true' ? (newVariablesOrContext as RequestContext) : context;

...
export const siteSchema = {
  slug: z
    ...
    .trim()
    .toLowerCase()
    .min(4, { message: '주소는 4글자 이상이여야 해요' })
    .max(63, { message: '주소는 63글자를 넘을 수 없어요' })
    .regex(/^[\da-z-]+$/, { message: '주소는 소문자, 숫자, 하이픈만 사용할 수 있어요' })
    .regex(/^[\da-z][\da-z-]*[\da-z]$/, { message: '주소는 하이픈으로 시작하거나 끝날 수 없어요' })
    .refine((str) => !str.includes('--'), { message: '하이픈을 연속으로 사용할 수 없어요' })
    .refine((str) => !UNAVAILABLE_SITE_SLUGS.EXACT.includes(str), { message: '사용할 수 없는 주소예요' }),
};
...
export const bootstrapSchema = z.object({
  ...
  maintenance: z.object({
    ...
    platforms: z.array(z.enum(['ios', 'android', 'web', 'api'])),

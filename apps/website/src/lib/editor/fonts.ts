...
const decodedMaps = new WeakMap<FontManifest, Uint8Array>();
    ...
    for (const variant of config.variants) {
      ...
      promises.push(
        loadOnce(`base:${config.family}:${variant.weight}`, async () => {
          ...
          app.addFontBase(config.family, variant.weight, new Uint8Array(buffer));
        }),
      );
    }
  }
      ...
      for (const idx of findChunkIndices(fm, codepoints)) {
        promises.push(
          loadOnce(`chunk:${config.family}:${variant.weight}:${idx}`, async () => {
            ...
            app.addFontChunk(config.family, variant.weight, new Uint8Array(buffer));
          }),
        );
      }
    }

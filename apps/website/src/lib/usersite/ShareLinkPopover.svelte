<script lang="ts">
  ...
  type Props = {
    ...
    iconSize?: 14 | 16 | 18 | 20;
  };

  let { href, style, iconSize = 14 }: Props = $props();
...
<button
  class={css({ marginLeft: 'auto', borderRadius: '4px', padding: '3px', _hover: { backgroundColor: 'surface.hover' } }, style)}
  aria-label="공유하기"
  onclick={() => (open = true)}
  type="button"
  use:anchor
>
  <Icon icon={ShareIcon} size={iconSize} />
</button>

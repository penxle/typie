<script lang="ts">
  ...
  let inflightName = $state<string>();
  let processedUploadId = $state<string>();

  const hasFile = $derived(!!fileData.src || !!inflightName);
  const isUploading = $derived(!!inflightName);
  ...
  const handleDownload = () => {
    if (fileData.src) {
      window.open(fileData.src, '_blank');
    }
  };

  const processFile = async (file: File) => {
    inflightName = file.name;
  ...
  const displayName = $derived(fileData.name || inflightName || '파일');
  const displaySize = $derived(typeof fileData.size === 'number' && fileData.size > 0 ? formatFileSize(fileData.size) : undefined);
</script>
...
>
  ...
  {#if el.isSelected}
    <div class={css({ position: 'absolute', inset: '0', backgroundColor: 'selection', pointerEvents: 'none', borderRadius: '8px' })}></div>
  {/if}
</div>

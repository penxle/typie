<script lang="ts">
  ...
  $effect(() => {
    open = editor.contextMenu.isOpen && editor.contextMenu.source === 'mouse';
  });

  $effect(() => {
    if (!open && editor.contextMenu.isOpen && editor.contextMenu.source === 'mouse') {
  ...
  const contextMenuPosition = $derived(
    editor.contextMenu.isOpen && editor.contextMenu.source === 'mouse' ? { x: editor.contextMenu.x, y: editor.contextMenu.y } : null,
  );
  const contextMenuPlacement = $derived(editor.contextMenu.placement);
  const contextMenuOffset = $derived(6);

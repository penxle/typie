<script lang="ts">
  ...
  const currentFontFamilyAndWeights = $derived.by(() => {
    ...
    const fontFamily = isMixed('font_family') ? undefined : ((findMark('font_family') as { family?: string })?.family ?? defaultFontFamily);
  ...
  const currentWeight = $derived(
    isMixed('font_weight') ? undefined : ((findMark('font_weight') as { weight?: number })?.weight ?? defaultFontWeight),
  );

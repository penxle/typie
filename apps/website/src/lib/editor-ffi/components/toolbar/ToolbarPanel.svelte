...
<script lang="ts">
  ...
  import { scrollFog, tooltip } from '@typie/ui/actions';
  ...
  const drill = $derived(title !== undefined);
  ...
  const recentItems = $derived(
    !drill && normalized.length === 0
      ? recentIds.map((id) => items.find((item) => item.id === id)).filter((item) => item !== undefined)
      : [],
  );
  ...
  const onKeydown = (e: KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      ...
      e.preventDefault();
      commit(active);
    } else if (onback && e.key === 'Backspace' && query.length === 0) {
      e.preventDefault();
      onback();
    }
  };
  ...
  const point = (index: number, event: PointerEvent) => {
    if (nav === 'keyboard' && !trackPointer(event)) return;
    active = index;
  };
...
>
  ...
  >
    {#if drill}
      ...
      <span class={css({ fontSize: '13px', fontWeight: 'semibold', color: 'text.default', whiteSpace: 'nowrap' })}>{title}</span>
    {:else}
        ...
        {#each entries as entry, index (entry.key)}
          {#if entry.zone !== 'strip'}
            {#if index === firstAction && firstAction > 0 && entries[firstAction - 1]?.zone !== 'strip'}
              <div
                class={css({ flexShrink: '0', marginX: '-4px', marginY: '4px', height: '1px', backgroundColor: 'border.hairline' })}
              ></div>
            {/if}
            <button
              id={`${listId}-${index}`}

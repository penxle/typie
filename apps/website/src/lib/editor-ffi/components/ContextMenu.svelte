<script lang="ts">
  ...
  $effect(() => {
    open = !!ctx.editor && ctx.editor.contextMenu.isOpen;
  });

  $effect(() => {
    if (open || !ctx.editor?.contextMenu.isOpen) {
      return;
    }
  ...
  const contextMenuPosition = $derived(
    ctx.editor?.contextMenu.isOpen ? { x: ctx.editor.contextMenu.x, y: ctx.editor.contextMenu.y } : null,
  );

export const PAGE_GAP = 24;
export const CROP_MARKER_SIZE = 32;
export const PAGINATED_HEADER_FOOTER_MIN_WIDTH = 320;
export const PAGINATED_HEADER_FOOTER_MIN_SCALE = 0.75;
export const CONTINUOUS_MIN_WIDTH = 300;
export const CONTINUOUS_VIEW_PADDING = 20;
export const PAGE_RENDER_OVERSCAN_MARGIN = '100% 0px';

export const CURSOR_VISIBLE_MARGIN = 60;
export const TYPEWRITER_MIN_BOTTOM_PADDING = 48;

export const IS_MAC = navigator.platform.toUpperCase().includes('MAC');

export const LONG_PRESS_MS = 500;
export const LONG_PRESS_CANCEL_DISTANCE_PX = 8;
export const NATIVE_CONTEXTMENU_SUPPRESS_AFTER_LONGPRESS_MS = 600;
export const EDGE_AUTO_SCROLL_THRESHOLD_PX = 60;
export const EDGE_AUTO_SCROLL_MIN_SPEED = 4;
export const EDGE_AUTO_SCROLL_MAX_SPEED = 16;
export const EDGE_AUTO_SCROLL_THROTTLE_MS = 16;
export const TOUCH_MENU_GAP = 10;
export const TOUCH_MENU_VIEWPORT_PADDING = 8;
export const TOUCH_MENU_ESTIMATED_HEIGHT = 56;
export const TAP_FEEDBACK_MIN_MS = 70;

<script lang="ts">
  ...
  import { SOCIAL_LINKS } from '$lib/landing/footer';
  ...
  const noteClass = css({ fontFamily: 'landing', fontSize: '13px', textAlign: 'center', color: 'text.hint' });
    ...
    >
      ...
      <nav class={navClass}>
        {#each NAV_LINKS as link (link.path)}
          <a class={linkClass} href={link.path} onclick={close}>{link.label}</a>
        {/each}
      </nav>

      <div class={footClass}>
        <a class={ctaClass} href="/start" onclick={close}>
          시작하기
          <Icon

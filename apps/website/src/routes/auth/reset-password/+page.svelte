...
<div class={flex({ flexDirection: 'column', gap: '24px' })}>
  ...
  <div class={flex({ flexDirection: 'column', gap: '4px' })}>
    ...
    <div class={css({ fontSize: { base: '13px', lg: '14px' }, color: 'text.muted' })}>새로운 비밀번호를 입력해주세요.</div>
  </div>
  ...
  <form class={flex({ flexDirection: 'column', gap: '24px' })} onsubmit={form.handleSubmit}>
    <div class={flex({ direction: 'column', gap: '12px' })}>
      <div class={flex({ direction: 'column', gap: '4px' })}>
        <label class={css({ fontSize: '13px', color: 'text.muted', userSelect: 'none' })} for="password">새 비밀번호</label>
        <TextInput
          id="password"

<script lang="ts">
  ...
  import type { UsersiteApexSlugPage_DocumentActionMenu_entityView$key } from '$mearie';

  type Props = {
    entityView$key: UsersiteApexSlugPage_DocumentActionMenu_entityView$key;
  };
  ...
  const entityView = createFragment(
    graphql(`
      fragment UsersiteApexSlugPage_DocumentActionMenu_entityView on EntityView {
        id
        slug
  ...
  const [reportDocument] = createMutation(
    graphql(`
      mutation UsersiteApexSlugPage_DocumentActionMenu_ReportDocument_Mutation($input: ReportDocumentInput!) {

<script lang="ts">
  ...
  import type { UsersiteApexSlugPage_FolderView_entityView$key } from '$mearie';

  type Props = {
    entityView$key: UsersiteApexSlugPage_FolderView_entityView$key;
  };
  ...
  const entityView = createFragment(
    graphql(`
      fragment UsersiteApexSlugPage_FolderView_entityView on EntityView {
        id
        slug
        url
      ...
      <header class={css({ marginBottom: { base: '56px', md: '72px' } })}>
        {#if folderAncestors.length > 0}
          <nav class={flex({ alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '20px' })}>
            {#each folderAncestors as ancestor, index (ancestor.id)}
              {#if ancestor.node.__typename === 'FolderView'}
                {#if index > 0}
                  <span class={css({ fontSize: '13px', color: 'text.hint' })}>/</span>
                {/if}
                <a class={css({ fontSize: '13px', color: 'text.hint', _hover: { color: 'text.muted' } })} href={`/s/${ancestor.slug}`}>
                  {ancestor.node.name}
                </a>
              {/if}
            {/each}
            ...
            {#each folders as entity (entity.id)}
              {#if entity.node.__typename === 'FolderView'}
                ...
                >
                  ...
                  <Icon style={css.raw({ flexShrink: '0', color: 'text.muted' })} icon={ChevronRightIcon} size={16} />
                </a>
              {/if}
            {/each}
          </div>

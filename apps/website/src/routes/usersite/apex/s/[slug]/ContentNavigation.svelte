<script lang="ts">
  ...
  import type { UsersiteApexSlugPage_ContentNavigation_entityView$key } from '$mearie';

  type Props = {
    entityView$key: UsersiteApexSlugPage_ContentNavigation_entityView$key;
  };
  ...
  const entityView = createFragment(
    graphql(`
      fragment UsersiteApexSlugPage_ContentNavigation_entityView on EntityView {
        id
  ...
  >
    {#if prevNode && entityView.data.prev}
      <a
        ...
        href={`/s/${entityView.data.prev.slug}`}
      >
    ...
    {#if nextNode && entityView.data.next}
      <a
        ...
        href={`/s/${entityView.data.next.slug}`}
      >

<script lang="ts">
  ...
  const items = $derived(
    publication.data.site.recentPublications.publications
      .filter((item) => item.id !== publication.data.id && item.id !== publication.data.next?.id)
      .slice(0, 4),
  );
</script>

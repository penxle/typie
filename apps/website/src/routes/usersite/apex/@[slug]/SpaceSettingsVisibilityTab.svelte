<script lang="ts">
  ...
  import type { UsersiteSpace_SpaceSettingsVisibilityTab_space$key } from '$mearie';

  type Props = {
    space$key: UsersiteSpace_SpaceSettingsVisibilityTab_space$key;
  };
  ...
  const space = createFragment(
    graphql(`
      fragment UsersiteSpace_SpaceSettingsVisibilityTab_space on Space {
        id
        allowIndexing
        allowDiscovery
      }
  ...
  const [updateSpace] = createMutation(
    graphql(`
      mutation UsersiteSpace_SpaceSettingsVisibilityTab_UpdateSpace_Mutation($input: UpdateSpaceInput!) {

<script lang="ts">
  ...
  import type { UsersiteSpace_SpaceSettingsGeneralTab_space$key } from '$mearie';

  type Props = {
    space$key: UsersiteSpace_SpaceSettingsGeneralTab_space$key;
  };
  ...
  const space = createFragment(
    graphql(`
      fragment UsersiteSpace_SpaceSettingsGeneralTab_space on Space {
        id
        name
        slug
  ...
  const [updateSpace] = createMutation(
    graphql(`
      mutation UsersiteSpace_SpaceSettingsGeneralTab_UpdateSpace_Mutation($input: UpdateSpaceInput!) {
  ...
  const [deleteSpace, deleteSpaceResult] = createMutation(
    graphql(`
      mutation UsersiteSpace_SpaceSettingsGeneralTab_DeleteSpace_Mutation($input: DeleteSpaceInput!) {

<script lang="ts">
  ...
  import type { SpaceDateDisplay, UsersiteWildcard_PinnedTile_publicationView$key } from '$mearie';

  type Props = {
    publicationView$key: UsersiteWildcard_PinnedTile_publicationView$key;
  ...
  const publication = createFragment(
    graphql(`
      fragment UsersiteWildcard_PinnedTile_publicationView on PublicationView {
        id
        title
        publishedAt
        updatedAt

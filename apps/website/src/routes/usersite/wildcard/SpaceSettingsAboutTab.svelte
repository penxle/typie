<script lang="ts">
  ...
  import type { UsersiteWildcard_SpaceSettingsAboutTab_space$key } from '$mearie';

  type Props = {
    space$key: UsersiteWildcard_SpaceSettingsAboutTab_space$key;
  };
  ...
  const space = createFragment(
    graphql(`
      fragment UsersiteWildcard_SpaceSettingsAboutTab_space on Space {
        id
        description
  ...
  const [updateSpace] = createMutation(
    graphql(`
      mutation UsersiteWildcard_SpaceSettingsAboutTab_UpdateSpace_Mutation($input: UpdateSpaceInput!) {

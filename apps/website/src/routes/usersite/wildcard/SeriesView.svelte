<script lang="ts">
  ...
  import type { SpaceDateDisplay, UsersiteWildcard_SeriesView_collectionView$key } from '$mearie';

  type Props = {
    collectionView$key: UsersiteWildcard_SeriesView_collectionView$key;
  ...
  const collection = createFragment(
    graphql(`
      fragment UsersiteWildcard_SeriesView_collectionView on CollectionView {
        id
        name
        description

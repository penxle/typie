<script lang="ts">
  ...
  beforeNavigate(({ from, to }) => {
    if (!from || !to) return;
    direction = Math.sign(order(to.url.pathname) - order(from.url.pathname)) || 1;
  ...
  const selectTab = (next: SpaceHomeTab) => {
    if (next === 'tags') {
      ...
      if (first) navigate(tagPath(first.name));
      return;
    }
    navigate(next === 'series' ? '/s' : '/');
  };

  const leaveCollection = () => navigate(isDesktop() ? '/' : '/s');
</script>
        ...
        {:else}
          ...
          {#if tag}
            <div class={css({ display: { base: 'none', lg: 'block' } })}>
              <BackBar label="글" onBack={() => navigate('/')} />
            </div>
          {:else}
            <div
              class={flex({

<script lang="ts">
  ...
  const [updateSpace, updateSpaceResult] = createMutation(
    graphql(`
      mutation UsersiteWildcard_SpaceSlugModal_UpdateSpace_Mutation($input: UpdateSpaceInput!) {
...
<Modal style={css.raw({ width: '480px', padding: '24px' })} bind:open>
  ...
  <form class={flex({ direction: 'column', gap: '20px' })} onsubmit={form.handleSubmit}>
    <div class={flex({ direction: 'column', gap: '6px' })}>
      ...
      <TextInput id="space-slug" autofocus rightItemAttached bind:value={form.fields.slug}>
        {#snippet rightItem()}
          <span
            class={css({
              fontSize: '13px',

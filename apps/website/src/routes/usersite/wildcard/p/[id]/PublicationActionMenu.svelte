<script lang="ts">
  ...
  import type { UsersiteWildcardPublicationPage_PublicationActionMenu_publicationView$key } from '$mearie';

  type Props = {
    publicationView$key: UsersiteWildcardPublicationPage_PublicationActionMenu_publicationView$key;
  };
  ...
  const publication = createFragment(
    graphql(`
      fragment UsersiteWildcardPublicationPage_PublicationActionMenu_publicationView on PublicationView {
        id
        documentId
        editUrl
        availableActions
  ...
  const [reportDocument] = createMutation(
    graphql(`
      mutation UsersiteWildcardPublicationPage_PublicationActionMenu_ReportDocument_Mutation($input: ReportDocumentInput!) {

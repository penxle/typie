<script lang="ts">
  ...
  let { open = $bindable(), entity }: Props = $props();
    ...
    <div class={flex({ flexDirection: 'column', gap: '16px' })}>
      <div class={flex({ flexDirection: 'column', gap: '12px' })}>
        <div class={flex({ justifyContent: 'space-between', alignItems: 'center', userSelect: 'none' })}>
          {#if entity.node.option.visibility === 'PRIVATE'}
      ...
      {#if entity.node.option.visibility === 'PRIVATE'}
        <Button
          ...
          onclick={async () => {
            if (entity.node?.__typename === 'Folder') await updateFolderOption({ folderId: entity.node.id, visibility: 'UNLISTED' });
          }}
        >
      ...
      {:else}
        <div class={flex({ justifyContent: 'flex-end' })}>
          <button
            ...
            onclick={async () => {
              if (entity.node?.__typename === 'Folder') await updateFolderOption({ folderId: entity.node.id, visibility: 'PRIVATE' });
            }}
            type="button"
          >

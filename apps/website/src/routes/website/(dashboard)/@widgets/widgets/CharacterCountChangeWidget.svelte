<script lang="ts">
  ...
  import type { Editor_Widget_CharacterCountChangeWidget_post } from '$graphql';

  type Props = {
    widgetId: string;
    palette?: boolean;
    editMode?: boolean;
    $post?: Editor_Widget_CharacterCountChangeWidget_post;
    data?: Record<string, unknown>;
  };

  let { widgetId, palette = false, editMode = false, $post: _post, data = {} }: Props = $props();
...
<Widget collapsed={isCollapsed} {editMode} icon={GoalIcon} {palette} title="오늘의 기록" {widgetId} widgetType="characterCountChange">

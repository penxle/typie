<script lang="ts">
  ...
  import type { Editor_Widget_PostRelatedNoteWidget_post } from '$graphql';

  type Props = {
    widgetId: string;
    palette?: boolean;
    editMode?: boolean;
    $post: Editor_Widget_PostRelatedNoteWidget_post | undefined;
  };

  let { widgetId, palette = false, editMode = false, $post: _post }: Props = $props();
...
<Widget {editMode} icon={StickyNoteIcon} noPadding {palette} title="이 포스트 관련 노트" {widgetId} widgetType="postRelatedNote">

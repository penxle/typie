...
>
    ...
    <div class={flex({ gap: '4px' })}>
      <button
        class={center({
          ...
          cursor: 'pointer',
          transition: 'common',
          color: note.status === 'RESOLVED' ? 'accent.success.default' : 'text.subtle',
          _hover: {
            color: note.status === 'RESOLVED' ? 'accent.success.default' : 'text.default',
        ...
        title={note.status === 'RESOLVED' ? '미해결로 변경' : '해결됨으로 변경'}
        type="button"
      >
        <Icon icon={note.status === 'RESOLVED' ? CircleCheckIcon : CircleIcon} size={16} />
      </button>
      <Popover
        style={center.raw({
          size: '24px',
  ...
  <p
    class={css({
      ...
      opacity: note.status === 'RESOLVED' ? '50' : '100',

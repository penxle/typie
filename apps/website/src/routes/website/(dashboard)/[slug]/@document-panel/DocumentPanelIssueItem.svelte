<script lang="ts">
  ...
  import type { DocumentPanelIssueItem_issue$key } from '$mearie';
  import type { IssuePriority, IssueStatus } from '../../@issues/constants';

  type Props = {
    issue$key: DocumentPanelIssueItem_issue$key;
    entityId: string;
    autoEdit?: boolean;
  };

  let { issue$key, entityId, autoEdit = false }: Props = $props();

  const issue = createFragment(
    graphql(`
      fragment DocumentPanelIssueItem_issue on Issue {
        id
        content
        status
        priority
  ...
  const [updateIssue] = createMutation(
    graphql(`
      mutation DocumentPanelIssueItem_UpdateIssue_Mutation($input: UpdateIssueInput!) {
  ...
  const [deleteIssue] = createMutation(
    graphql(`
      mutation DocumentPanelIssueItem_DeleteIssue_Mutation($input: DeleteIssueInput!) {
  ...
  const handleClickOutside = (e: MouseEvent) => {
    ...
    if (
      ...
      return;
    if (!target.isConnected || !target.closest(`[data-issue-item="${issue.data.id}"]`)) {
      editing = false;
    }
  };
...
>
  ...
  {:else}
    <!-- View mode -->
    <div class={flex({ flexDirection: 'column', padding: '10px', cursor: 'pointer', userSelect: 'none' })}>

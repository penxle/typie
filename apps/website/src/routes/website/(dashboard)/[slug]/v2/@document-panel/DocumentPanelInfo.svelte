<script lang="ts">
  ...
  type Props = {
    document$key: DocumentPanelV2_Info_document$key;
    user$key: DocumentPanelV2_Info_user$key;
  ...
  const document = createFragment(
    graphql(`
      fragment DocumentPanelV2_Info_document on Document {
        id
        createdAt
        updatedAt
  ...
  const user = createFragment(
    graphql(`
      fragment DocumentPanelV2_Info_user on User {
        id
      }
    `),
    () => user$key,

...
<details class={flex({ flexDirection: 'column', marginBottom: open ? '12px' : '8px' })} bind:open>
  <summary class={flex({ alignItems: 'center', gap: '4px', cursor: 'pointer', marginBottom: open ? '8px' : '0', userSelect: 'none' })}>
    <Icon style={{ color: 'gray.500' }} icon={IconType} size={12} />
    ...
    {#if !open}
      <div class={css({ fontSize: '13px', fontWeight: 'medium', color: 'gray.700' })} in:fly={{ y: 2, duration: 150 }}>
  ...
  {#if open}
    <div class={flex({ flexDirection: 'column', gap: '2px' })} in:fly={{ y: -2, duration: 150 }}>
      <dl class={flex({ justifyContent: 'space-between', gap: '8px', fontSize: '13px' })}>
        ...
        <dd class={css({ fontWeight: 'medium', color: 'gray.700' })}>{comma(countWithWhitespace)}자</dd>
      </dl>

      <dl class={flex({ justifyContent: 'space-between', gap: '8px', fontSize: '13px' })}>
        <dt class={css({ color: 'gray.500' })}>공백 미포함</dt>
        <dd class={css({ fontWeight: 'medium', color: 'gray.700' })}>{comma(countWithoutWhitespace)}자</dd>
      </dl>

      <dl class={flex({ justifyContent: 'space-between', gap: '8px', fontSize: '13px' })}>
        <dt class={css({ color: 'gray.500' })}>공백/부호 미포함</dt>
        <dd class={css({ fontWeight: 'medium', color: 'gray.700' })}>{comma(countWithoutWhitespaceAndPunctuation)}자</dd>
      </dl>
    </div>
  {/if}
</details>

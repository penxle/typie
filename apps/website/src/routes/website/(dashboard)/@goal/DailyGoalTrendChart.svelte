<script lang="ts">
  ...
  import type { DashboardLayout_DailyGoalTrendChart_user$key } from '$mearie';

  type Props = { user$key: DashboardLayout_DailyGoalTrendChart_user$key };

  let { user$key }: Props = $props();

  const user = createFragment(
    graphql(`
      fragment DashboardLayout_DailyGoalTrendChart_user on User {
        id

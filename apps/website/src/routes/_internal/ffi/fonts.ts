...
async function loadBase(family: string, font: FontRef): Promise<void> {
  await loadOnce(`base:${family}:${font.weight}`, async () => {
    ...
    wasm.load_font_base(family, font.weight, buffer);
  });
}

async function loadChunks(family: string, font: FontRef, codepoints: number[]): Promise<void> {
  const manifest = await fetchManifest(font.url);
  ...
  await Promise.allSettled(
    findChunkIndices(manifest, codepoints).map((idx) =>
      loadOnce(`chunk:${family}:${font.weight}:${idx}`, async () => {
        ...
        wasm.load_font_chunk(family, font.weight, buffer);
      }),
    ),
  );
}
...
export async function initFonts(): Promise<void> {
  ...
  wasm.set_fallback_font_families(phantomFontFamilies.map((v) => v.familyName));
...
export async function ensureRequiredFont(family: string, font: FontRef, codepoints: number[]): Promise<void> {
  await loadBase(family, font);
  await loadChunks(family, font, codepoints);
}

export async function preloadRemainingChunks(family: string, font: FontRef): Promise<void> {
  try {
    const manifest = await fetchManifest(font.url);
...
export async function resolveFallbackMappings(
  ...
  for (const fallbackFontFamily of fallbackFontFamilies) {
    ...
    await loadBase(fallbackFontFamily.familyName, fontRef);
    await loadChunks(fallbackFontFamily.familyName, fontRef, covered);

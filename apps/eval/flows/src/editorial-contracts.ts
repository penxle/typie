// Editorial 파이프라인의 도구·검수 스키마 — 스펙 §4~§8의 계약을 wire로 옮긴 것.
// 배열 길이 방지선(축 개수 등)은 strict 제약 디코딩 지원이 불확실해 여기 적지 않고
// editorial-checks가 코드로 강제한다.
...
export const READ_TOOL: Anthropic.Messages.Tool = strict({
  name: 'read',
  description: '원고의 구간을 연다. 문자 좌표 기준이며 한 번에 최대 4,000자. 잘리면 truncated가 참으로 온다.',
  input_schema: {
    type: 'object',
    properties: {
      start: { type: 'number', description: '시작 문자 좌표(0 기준)' },
      end: { type: 'number', description: '끝 문자 좌표(미포함)' },
    },
    required: ['start', 'end'],
...
export const GREP_TOOL: Anthropic.Messages.Tool = strict({
  name: 'grep',
  description:
    '원고 전문에서 정규식 매치 전건의 좌표와 주변 문맥을 돌려준다. 한국어 특성상 활용·띄어쓰기·표기 변형을 고려해 한 확인에 여러 변형 패턴을 검색하라(어간·부분 문자열 우선). 무매치는 부재의 증거가 아니다 — 패턴이 틀렸을 가능성을 항상 전제하라.',
  input_schema: {
    type: 'object',
    properties: { pattern: { type: 'string', description: '정규식(u 플래그)' } },
    required: ['pattern'],
    additionalProperties: false,
  },
});
...
export const SUBMIT_RESEARCH_TOOL: Anthropic.Messages.Tool = strict({
  name: 'submit_research',
  description: '조사 결과를 확정 제출한다. 모든 인용은 원고에서 글자 그대로여야 하고, 열람한 범위 안이어야 한다.',
  input_schema: {
    type: 'object',
    properties: {
      nature: {
        ...
        properties: {
          ...
          conventions: {
            ...
            items: {
              ...
              properties: {
                pattern: { type: 'string' },
                evidence: { type: 'array', description: '원고에서 글자 그대로 복사한 인용', items: { type: 'string' } },
              },
      ...
      boundaries: {
        ...
        items: {
          type: 'object',
          properties: {
            startQuote: { type: 'string', description: '구간 시작 문구를 원고에서 그대로 인용' },
            endQuote: { type: 'string', description: '구간 끝 문구를 원고에서 그대로 인용' },
            reason: { type: 'string' },
          },
...
export const SUBMIT_PLAN_TOOL: Anthropic.Messages.Tool = strict({
  name: 'submit_plan',
  description: '비평 계획을 제출한다. 축 개수는 이 글의 위험 프로파일이 정한다.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      protected: {
        ...
        items: {
          ...
          properties: {
            ...
            evidence: { type: 'array', description: 'read로 채집한 원고 인용 — 열람 범위 검증 대상', items: { type: 'string' } },
      ...
      axes: {
        type: 'array',
        description: '원고 전체에 적용할 검토 관점. 개수는 이 글의 위험 프로파일이 정한다',
        items: {
          type: 'object',
          properties: {
          ...
          required: ['label', 'inquiry', 'risk', 'evidence', 'conventionsCheck', 'conventionsBasis'],
          additionalProperties: false,
        },
      },
      verifications: {
...
// 축 enum이 스테이지·문서마다 다르므로 도구를 호출부가 짓는다. 암묵 추가는 없다.
export const fileFindingTool = (axes: string[]): Anthropic.Messages.Tool => {
  if (axes.length === 0) throw new Error('fileFindingTool: 축이 비어 있다');
  return strict({
    name: 'file_finding',
    description: '읽다가 걸린 지적 하나를 그 자리에서 제출한다. 앵커는 읽은 범위 안이어야 한다.',
    input_schema: {
      type: 'object',
      properties: {
        axis: { type: 'string', enum: axes },
        quoteStart: { type: 'string', description: '대목 시작 문구를 원고에서 그대로 인용 (한 문장 안팎, 위치가 유일하게 식별되게)' },
        quoteEnd: { type: 'string', description: '끝 문구를 원고에서 그대로 인용 (한 문장 안팎, 위치가 유일하게 식별되게)' },
...
export const FILE_STRENGTH_TOOL: Anthropic.Messages.Tool = strict({
  name: 'file_strength',
  description: '잘 작동하는 대목 하나를 제출한다. 조언·확장 제안 금지 — 총평의 재료다.',
  input_schema: {
    type: 'object',
    properties: {
      quoteStart: { type: 'string', description: '대목 시작 문구를 원고에서 그대로 인용' },
      quoteEnd: { type: 'string', description: '끝 문구를 원고에서 그대로 인용' },
      principle: { type: 'string', description: '무엇이 왜 작동하는지' },
    },
...
export const SUBMIT_REVIEW_TOOL: Anthropic.Messages.Tool = strict({
  name: 'submit_review',
  description: '검토 종료를 선언한다. 열람 범위가 본문 전체를 덮지 않으면 반려된다.',
  input_schema: {
    type: 'object',
...
export const EDITORIAL_COMPOSE_TOOL: Anthropic.Messages.Tool = strict({
  name: 'compose_feedbacks',
  description: '지적들을 작가에게 나갈 피드백 문면으로 확정한다. 병합은 같은 축·인접 앵커끼리만.',
  input_schema: {
    type: 'object',
    properties: {
      feedbacks: {
...
export const EDITORIAL_COMPOSE_REVIEW_TOOL: Anthropic.Messages.Tool = strict({
  name: 'report_review',

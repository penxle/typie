<script lang="ts">
  ...
  const flagged = untrack(() => new SvelteSet<string>(data.draft?.falsePositiveFeedbackIds as string[] | undefined));
  ...
  let ranks = $state<Record<string, number>>(
    untrack(() =>
      draftResult?.kind === 'ranking'
        ? Object.fromEntries(draftResult.ranks.map((r) => [r.setId, r.rank]))
        : Object.fromEntries(data.task.setIds.map((setId) => [setId, 0])),
    ),
  );
...
<div class={css({ height: '[100dvh]', display: 'flex', flexDirection: 'column', backgroundColor: 'surface.subtle' })}>
  ...
  >
    ...
    <div class={flex({ align: 'center', gap: '8px' })}>
      <span class={css({ fontSize: '13px', color: 'text.subtle' })}>내 판정 {data.progress.done} / 태스크 {data.progress.total}</span>
      <div class={css({ width: '120px', height: '4px', borderRadius: 'full', backgroundColor: 'surface.muted', overflow: 'hidden' })}>
        <div
          style:width={`${data.progress.total === 0 ? 0 : Math.round((data.progress.done / data.progress.total) * 100)}%`}

...
import { applyEvent, capsuleLabel, groupFeed, initialLive, pausedMs } from './live.ts';
...
const asked = (id: number, agentId = 'agent-a', at = 1000 + id) =>
  ev(
    id,
    'tool.requested',
    { agent: { id: agentId, name: 'rubric' }, turn: 3, attempt: 1, tool: 'ask-user', toolCallId: 'call_1', input: ASK_INPUT },
    at,
  );
...
describe('live reducer', () => {
  it('step 이벤트로 스테이지 상태를 전이한다', () => {
    ...
    s = applyEvent(s, ev(2, 'step.started', { step: 'description-0' }));
    expect(s.stages.description).toBe('running');
    s = applyEvent(s, ev(3, 'step.completed', { step: 'description-0' }));
    s = applyEvent(s, ev(4, 'step.started', { step: 'rubric-0' }));
  ...
  it('표시 스테이지가 없는 step은 상태를 흔들지 않는다', () => {
    let s = initialLive([ev(1, 'step.started', { step: 'judgment-0' })]);
  ...
  it('빈 문자열 text는 없는 것과 같다 — 라인을 만들지 않는다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'judgment-0' }),
  ...
  it('턴 발화도 진행 중 스테이지의 시각을 앞으로 민다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'judgment-0' }, 1000),
  ...
  it('tool.called은 라인을 만들지 않고 진행 시각만 남긴다', () => {
    let s = initialLive([ev(1, 'step.started', { step: 'judgment-0' }, 1000)]);
  ...
  it('tool.called은 캡슐 자취(mark)로 남는다 — 실패 호출과 list는 빠진다', () => {
    let s = initialLive([ev(1, 'step.started', { step: 'description-0' }, 1000)]);
    s = applyEvent(
      s,
  ...
  it('calibration 스텝의 발화는 그 스텝에 귀속된다', () => {
    let s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }), ev(2, 'step.started', { step: 'calibration-1' })]);
    s = applyEvent(
      s,
  ...
  it('스테이지 전이는 활동 라인을 만들지 않는다', () => {
    const s = initialLive([ev(1, 'step.started', { step: 'judgment-0' }), ev(2, 'step.started', { step: 'stylistic-0' })]);
  ...
  it('계획 검토·재수립 스텝은 하니스 라인을 만들지 않는다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'rubric-0' }),
      ev(2, 'step.started', { step: 'calibration-1-0' }),
      ev(3, 'step.started', { step: 'rubric-revise-1' }),
    ]);
    expect(s.activity).toHaveLength(0);
    expect(s.stages.rubric).toBe('running');
  ...
  it('스테이지별 firstAt·lastAt을 봉투 시각으로 추적한다', () => {
    const s = initialLive([
      ...
      ev(2, 'step.started', { step: 'description-0' }, 2000),
      ev(3, 'tool.called', { agent: AG, turn: 1, attempt: 1, tool: 'read', ok: true }, 5000),
      ev(4, 'step.started', { step: 'rubric-0' }, 9000),
    ]);
  ...
  it('workflow.completed는 진행 중이던 스테이지를 닫고, 실패·취소는 멈춘 자리를 남긴다', () => {
    const seeded = initialLive([ev(1, 'step.started', { step: 'delivery-0' }, 4000)]);
    ...
    expect(applyEvent(seeded, ev(2, 'workflow.failed', { reason: 'boom' })).stages.delivery).toBe('running');
  ...
  it('커서 이하의 재생분은 다시 접지 않는다', () => {
    ...
    const seeded = initialLive([ev(1, 'step.started', { step: 'judgment-0' }), ev(2, 'turn.completed', line)]);
  ...
  it('검수 라운드의 자취는 발화와 무관하게 남는다 — 다음 스텝이 그 라운드를 닫는다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'rubric-0' }, 1000),
      ev(2, 'step.started', { step: 'calibration-1-0' }, 2000),
      ev(3, 'step.started', { step: 'calibration-1-1' }, 60_000),
      ev(4, 'step.started', { step: 'rubric-revise-1' }, 125_000),
    ]);
    expect(s.activity).toHaveLength(0);
  ...
  it('종결 이벤트는 돌던 검수 라운드를 그 시각으로 닫는다', () => {
    const seeded = initialLive([
      ev(1, 'step.started', { step: 'rubric-0' }, 1000),
      ev(2, 'step.started', { step: 'calibration-2-0' }, 2000),
    ]);
  ...
  it('시각 없는 봉투로 열린 라운드도 자취는 남는다 — 시간만 비어 있다', () => {
    const naked: SseEvent = {
      id: 1,
      event: 'step.started',
      data: JSON.stringify({ seq: 1, kind: 'step.started', data: { step: 'calibration-1-0' } }),
    };
...
describe('질문 상태 기계', () => {
  it('tool.requested가 pending 엔트리를 세우고 스테이지·스텝에 귀속한다', () => {
    const s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2)]);
  ...
  it('같은 agent의 ask-user tool.called가 pending을 answered로 굳힌다 — ok:false여도', () => {
    for (const ok of [true, false]) {
      const s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2), answered(3, 'agent-a', ok)]);
  ...
  it('타 agent·타 도구의 tool.called는 pending을 건드리지 않는다', () => {
    const other = ev(
      3,
      'tool.called',
      { agent: { id: 'agent-b', name: 'rubric' }, turn: 1, attempt: 1, tool: 'read', input: { path: 'manuscript/01.txt' }, ok: true },
      3000,
    );
    const s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2), other, answered(4, 'agent-b')]);
    expect(s.questions[0].status).toBe('pending');
  });

  it('requested 없는 ok:false tool.called(파킹 전 검증 실패)는 무시된다', () => {
    const s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), answered(2, 'agent-a', false)]);
    expect(s.questions).toHaveLength(0);
  });

  it('루트 종결이 pending을 closed로 굳힌다', () => {
    const seeded = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2)]);
  ...
  it('순차 질문의 기록이 배열로 보존된다 — research 답변 후 plan 질문', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'description-0' }, 1000),
      asked(2, 'agent-r'),
      answered(3, 'agent-r'),
      ev(4, 'step.started', { step: 'rubric-0' }, 4000),
      asked(5, 'agent-p'),
    ]);
  ...
  it('깨진 input은 엔트리를 만들지 않는다', () => {
    const broken = ev(
      2,
      'tool.requested',
      { agent: { id: 'agent-a', name: 'rubric' }, turn: 1, attempt: 1, tool: 'ask-user', toolCallId: 'call_1', input: '{broken' },
      2000,
    );
    expect(initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), broken]).questions).toHaveLength(0);
  });
  ...
  it('빈 문면(hint·question·label)도 엔트리를 세운다 — 카드가 없으면 파킹된 run은 답할 길이 없다', () => {
    ...
    const s = initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), bare]);
    expect(s.questions).toHaveLength(1);
  ...
  it('구조가 어긋난 질문은 여전히 엔트리를 만들지 않는다', () => {
    ...
    const seed = ev(1, 'step.started', { step: 'rubric-0' }, 1000);
  ...
  it('ask-user 아닌 도구의 tool.requested는 질문이 아니다', () => {
    ...
    expect(initialLive([ev(1, 'step.started', { step: 'rubric-0' }, 1000), parked]).questions).toHaveLength(0);
  });

  it('재생 복원 동형성 — 같은 이벤트열은 같은 상태를 만든다', () => {
    const events = [ev(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2), answered(3)];
...
describe('feed grouping', () => {
  ...
  it('발화 없는 검수 라운드도 빈 카드로 선다 — 자리는 라운드가 시작한 순간이다', () => {
    const s = initialLive([
      ...
      ev(3, 'step.started', { step: 'calibration-1-0' }, 3000),
      ev(4, 'step.started', { step: 'rubric-revise-1' }, 4000),
  ...
  it('같은 라운드의 연속 발화는 한 카드로 접히고, 라운드가 갈리면 카드도 갈린다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'calibration-1-0' }, 1000),
      talk(2, '첫 검수 발화', 2000),
      ev(3, 'step.started', { step: 'calibration-1-1' }, 3000),
      talk(4, '이어진 검수 발화', 4000),
      ev(5, 'step.started', { step: 'rubric-revise-1' }, 5000),
      talk(6, '되짚어 고쳐 적었어요', 6000),
      ev(7, 'step.started', { step: 'calibration-2-0' }, 7000),
  ...
  it('질문은 제 커서 자리에 카드로 선다 — 라운드 안이면 그 카드 안이다', () => {
    const s = initialLive([
      ...
      ev(5, 'step.started', { step: 'calibration-1-0' }, 5000),
  ...
  it('질문을 기다린 시간은 생각으로 세지 않는다 — 사람이 기다린 틈이다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'rubric-0' }, 0),
  ...
  it('라운드 자취는 제 스테이지 밖으로 새지 않는다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'calibration-1-0' }, 1000),
      ev(2, 'step.started', { step: 'judgment-0' }, 2000),
  ...
  it('발화 사이의 연속 도구 자취가 캡슐로 접히고, 조용한 틈은 생각으로 선다', () => {
    const s = initialLive([
      ev(1, 'step.started', { step: 'description-0' }, 0),
  ...
  it('같은 상태를 다시 접어도 같은 구조가 선다', () => {
    const events = [
      ev(1, 'step.started', { step: 'calibration-1-0' }, 1000),
      talk(2, '검수 발화', 2000),
      ev(3, 'step.started', { step: 'calibration-2-0' }, 3000),
    ];
...
describe('question paused time', () => {
  ...
  const agent = { id: 'agent-a', name: 'rubric' };
  ...
  it('해소·종결이 settledAt을 이벤트 시각으로 굳힌다', () => {
    const base = [frame(1, 'step.started', { step: 'rubric-0' }, 1000), asked(2, 2000)];
...
describe('재개(workflow.retried)', () => {
  it('실패로 굳은 terminal을 풀고 실패~재개 구간을 파킹으로 접는다 — 멈춘 스테이지는 그대로 이어진다', () => {
    let s = initialLive([
      ev(1, 'workflow.started', {}, 1000),
      ev(2, 'step.started', { step: 'judgment-0' }, 2000),
      ev(3, 'workflow.failed', { error: 'boom' }, 10_000),
    ]);
    expect(s.terminal).toBe(true);
    expect(s.failedAt).toBe(10_000);
    expect(s.stages.judgment).toBe('running');

    s = applyEvent(s, ev(4, 'workflow.retried', { reissued: [] }, 70_000));
  ...
  it('실패가 반복되면 구간도 누적된다', () => {
    const s = initialLive([
      ...
      ev(3, 'workflow.retried', { reissued: [] }, 20_000),
      ev(4, 'workflow.failed', {}, 30_000),
      ev(5, 'workflow.retried', { reissued: [] }, 45_000),
    ]);
  ...
  it('봉투 시각을 잃은 실패는 감산을 포기한다 — 재개는 그래도 terminal을 푼다', () => {
    ...
    s = applyEvent(s, {
      ...
      data: JSON.stringify({ seq: 2, kind: 'workflow.failed', data: {}, createdAt: null }),
    });
    expect(s.failedAt).toBeNull();
    s = applyEvent(s, ev(3, 'workflow.retried', { reissued: [] }, 20_000));
    expect(s.terminal).toBe(false);
    expect(s.pauses).toEqual([]);
  });
});
...
const turnDone = (id: number, agent: string, usage: object | null, text: string | null = null) =>
  ev(id, 'turn.completed', { agent: { id: `agent-${agent}`, name: agent }, turn: 1, attempt: 1, text, thinking: null, usage });

describe('usage 누적', () => {
  it('턴 usage를 에이전트별로 접는다 — 도구만 부른 턴(text null)도 접는다', () => {
    let s = initialLive([]);
    s = applyEvent(
      s,

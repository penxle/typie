...
import { EvaluatorConsents, Judgments, Rounds, Tasks } from './db/index.ts';
...
// 이때 필요 수는 어드민을 뺀 동의 평가자 수가 된다. 1로 세면 첫 판정에 100%로 잠긴다.
// 범위는 최신 라운드 하나다 — 전 라운드 합산은 새 라운드 시작 직후 진행률을 부풀린다.
export const effectiveProgress = async (db: Db, adminEmails = ''): Promise<{ done: number; required: number; roundId: string | null }> => {

<script lang="ts">
  ...
  const { items, numbers, stageKey, artifacts, focusedId = null, onHover, onSelect, onReveal, control, runReview }: Props = $props();

  // 산출물 탭은 리서치·계획 평가 단계와 판정 없는 열람에서만 연다 — 작품 판정 단계에서
  // 먼저 보면 판정이 오염된다. 이 결정은 이 세대의 것이라 여기서 내린다.
  const showArtifactsTab = $derived(artifacts !== null && (stageKey === null || stageKey === 'artifacts'));

  let topTab = $state<'feedback' | 'artifacts'>('feedback');
  let artifactTab = $state<'research' | 'plan'>('research');

  // 리서치·계획 평가 단계에 들어서면 산출물 탭을 바로 연다 — 읽는 것이 이 단계의 일이다.
  $effect(() => {
    if (stageKey === 'artifacts' && artifacts) topTab = 'artifacts';
  });
  ...
  const cleared = $derived(of('cleared'));
  const patterns = $derived(of('pattern'));
  const priority = $derived(of('priority'));
  const findings = $derived(of('finding'));

  // layer facet으로 나눈다. 층위가 없는 항목은 작품 검토로 보낸다.
  const planFindings = $derived(findings.filter((f) => f.facets.layer !== 'local'));
  const localFindings = $derived(findings.filter((f) => f.facets.layer === 'local'));
  ...
  // 카드로 이동. 총평 항목이면 총평 탭에서, 지적이면 층위 탭에서 세운다 — 어느 쪽이든
  // 다른 탭의 스크롤 위치는 그대로 남는다.
  export const focus = (itemId: string) => {
    topTab = 'feedback';
    const isFinding = items.some((i) => i.id === itemId && i.kind === 'finding');
    if (!isFinding) {
      switchTab('review');
      requestAnimationFrame(() => {
        const el = document.querySelector<HTMLElement>(`[data-item-card="${CSS.escape(itemId)}"]`);
        if (el && reviewPaneEl) scrollWithin(reviewPaneEl, el);
      });
      return;
    }
...
<div class={flex({ direction: 'column', minHeight: '0', height: 'full' })}>
  {#if showArtifactsTab && artifacts}
    <div class={flex({ paddingX: '16px', borderBottomWidth: '1px', borderColor: 'border.default', flexShrink: '0' })}>
      <button class={tabClass(topTab === 'feedback')} onclick={() => (topTab = 'feedback')} type="button">피드백</button>
      <button class={tabClass(topTab === 'artifacts')} onclick={() => (topTab = 'artifacts')} type="button">{artifacts.label}</button>
    </div>
  {/if}

  <div style:display={topTab === 'feedback' ? 'flex' : 'none'} class={flex({ direction: 'column', flex: '1', minHeight: '0' })}>
    <div class={flex({ paddingX: '16px', borderBottomWidth: '1px', borderColor: 'border.default', flexShrink: '0' })}>
    ...
    >
      {#if characterization}
        <section class={sectionClass} data-item-card={characterization.id}>
          <h2 class={headingClass}>이 작품을 이렇게 읽었습니다</h2>
          <p class={bodyClass}>{characterization.body}</p>
          {#if control}
            <div class={css({ marginTop: '10px' })}>{@render control(characterization)}</div>
          {/if}
        </section>
      {/if}
          ...
          <div class={flex({ direction: 'column', gap: '16px' })}>
            {#each strengths as item (item.id)}
              <div data-item-card={item.id}>
                ...
                {/if}
                {#if control}
                  <div class={css({ marginTop: '10px' })}>{@render control(item)}</div>
                {/if}
              </div>
            {/each}
          </div>
      ...
      {#if cleared.length > 0}
        <section class={sectionClass}>
          ...
          <div class={flex({ direction: 'column', gap: '16px' })}>
            {#each cleared as item (item.id)}
              <div data-item-card={item.id}>
                {#if item.facets.axis}
                  <h3 class={subHeadingClass}>{item.facets.axis}</h3>
                {/if}
                <p class={bodyClass}>{item.body}</p>
                {#if control}
                  <div class={css({ marginTop: '10px' })}>{@render control(item)}</div>
        ...
        >
          ...
          <div class={css({ minWidth: '0' })}>
            <div class={flex({ align: 'center', gap: '8px', marginBottom: '4px', minHeight: '22px' })}>
              {#if item.facets.axis}
                <span class={css({ fontSize: '12px', fontWeight: 'bold', color: 'text.subtle' })}>{item.facets.axis}</span>
              {/if}
  ...
  {#if showArtifactsTab && artifacts}
    <div style:display={topTab === 'artifacts' ? 'flex' : 'none'} class={flex({ direction: 'column', flex: '1', minHeight: '0' })}>
      <div class={flex({ paddingX: '16px', borderBottomWidth: '1px', borderColor: 'border.default', flexShrink: '0' })}>
        <button class={tabClass(artifactTab === 'research')} onclick={() => (artifactTab = 'research')} type="button">리서치</button>
        <button class={tabClass(artifactTab === 'plan')} onclick={() => (artifactTab = 'plan')} type="button">비평 계획</button>
      </div>
      <div class={css({ overflowY: 'auto', flex: '1', minHeight: '0', padding: '20px' })}>
        <ArtifactsView tab={artifactTab} value={artifacts.value} />
      </div>
    </div>
  {/if}
</div>

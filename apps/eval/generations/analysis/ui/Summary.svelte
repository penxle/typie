<script lang="ts">
  ...
  const itemFields = TRIAXIAL.stages[0].items[0].fields.filter((f) => (f.render as AnalysisRender).kind === 'yesNo');
  const runFields = TRIAXIAL.stages[0].run.filter((f) => (f.render as AnalysisRender).kind === 'yesNo');

  const renderOf = (field: (typeof itemFields)[number]) => field.render as Extract<AnalysisRender, { kind: 'yesNo' }>;
  ...
  const tally = (values: unknown[]) => ({ yes: values.filter((v) => v === true).length, no: values.filter((v) => v === false).length });
  ...
  const itemTallies = $derived(
    itemFields.map((f) => ({ key: f.key, question: renderOf(f).question, tally: tally(allEntries.map((e) => e.payload[f.key])) })),
  );
  ...
  const rejections = $derived(
    confirmed
      .flatMap((judgment) =>
        judgment.entries
          .filter((entry) => itemFields.some((f) => entry.payload[f.key] === false))
          .map((entry) => {
            const found = itemById.get(entry.itemId);
            return {
              key: `${judgment.id}:${entry.itemId}`,
              itemId: entry.itemId,
              number: numberOf.get(entry.itemId) ?? 0,
              refId: found?.run.refId ?? '?',
              taskId: found?.run.taskId ?? null,
              body: found?.item.body ?? '',
              axis: found?.item.facets.category ?? '',
              evaluator: judgment.evaluatorEmail,
              failed: Object.fromEntries(itemFields.map((f) => [f.key, entry.payload[f.key] === false])),
  ...
  const reviewNotes = $derived(
    confirmed
      ...
      .filter((r) => Object.values(r.failed).some(Boolean) || r.note || r.comment),
  );
    ...
    view.runs
      .map((run) => {
        ...
        return {
          ...
          no: entries.filter((e) => itemFields.some((f) => e.payload[f.key] === false)).length,
        };
      })
      .filter((d) => d.judged > 0)
  ...
  const counts = $derived(
    Object.fromEntries([
      ...
      ...itemFields.map((f) => [f.key, rejections.filter((r) => r.failed[f.key]).length] as const),
    ]),
  );
                ...
                <article class={entryClass}>
                  ...
                  <div class={flex({ align: 'center', gap: '8px', flexWrap: 'wrap' })}>
                    ...
                    <span class={flex({ align: 'center', gap: '10px', marginLeft: 'auto' })}>
                      ...
                      <AxisMark slots={itemFields.map((f) => ({ label: renderOf(f).short, failed: item.failed[f.key] }))} />
                    </span>
                  </div>

...
internal class RelatedNotesViewModel(private val entityId: String) : ViewModel() {
  ...
  val query =
    Apollo.watchQuery(scope = viewModelScope, resetOnChange = false) {
      RelatedNotesSheet_Query(entityId = entityId, status = filterStatus)
    }
  ...
  suspend fun createNote(color: String = DEFAULT_NOTE_COLOR): Result<NoteCard_note, Nothing> =
    createNoteMutation(color = color, entityId = entityId)

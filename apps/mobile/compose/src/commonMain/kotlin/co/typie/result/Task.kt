...
class Task<P, T, E> internal constructor(private val flow: Flow<Event<P, T, E>>) {
  ...
  suspend fun collect(onPending: suspend (P) -> Unit, onSettled: suspend (Result<T, E>) -> Unit) =
    flow.collect { event ->
      when (event) {
...
class TaskScope<P, E> @PublishedApi internal constructor(private val emitFn: suspend (P) -> Unit) {
  ...
  @PublishedApi internal class RaiseException(val error: Any?) : Exception()
}
  ...
  val flow = channelFlow {
    ...
    val result: Result<T, E> =
      try {
        ...
        @Suppress("UNCHECKED_CAST") Result.Err(e.error as E)

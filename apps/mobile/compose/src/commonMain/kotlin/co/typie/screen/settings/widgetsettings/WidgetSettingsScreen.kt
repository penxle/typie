package co.typie.screen.settings.widgetsettings
...
fun WidgetSettingsScreen() {
  ...
  Screen { contentPadding ->
    Column(
      ...
      Text("위젯", style = AppTheme.typography.display)
      ...
      WidgetSettingsSection(title = "위젯 설정") {
        SettingControlRow(
          ...
          onClick = {
            Preference.characterCountFloatingEnabled = !Preference.characterCountFloatingEnabled
          },
          trailing = {
            SettingSwitch(
              checked = Preference.characterCountFloatingEnabled,
              onCheckedChange = { next -> Preference.characterCountFloatingEnabled = next },
            )
          },
        )

        AnimatedVisibility(visible = Preference.characterCountFloatingEnabled) {
          CardDivider(inset = 20.dp)

          SettingControlRow(
            label = "위젯 자동 페이드 인/아웃",
            description = "타이핑, 스크롤 시 위젯이 잠시 사라집니다.",
            onClick = { Preference.widgetAutoFadeEnabled = !Preference.widgetAutoFadeEnabled },
            trailing = {
              SettingSwitch(
                checked = Preference.widgetAutoFadeEnabled,

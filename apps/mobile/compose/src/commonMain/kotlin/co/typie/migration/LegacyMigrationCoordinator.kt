...
object LegacyMigrationCoordinator : LegacyMigrationRunner {
  override suspend fun runIfNeeded(): LegacyMigrationRunResult {
    ...
    val source =
      PlatformModule.legacyMigrationPlatformSource.load()
        ?: return LegacyMigrationRunResult(
          ...
          .also { Logger.i { "Legacy migration: source missing." } }
    ...
    val prefsResult =
      takeIf { source.preferenceBox != null || source.themeBox != null }
  ...
  private fun importAuth(source: LegacyEncryptedHiveBoxSource): LegacyMigrationStepResult {
    return runCatching {
        ...
        val sessionToken =
          authValues["session_token"] as? String
            ?: error("Missing session_token in legacy auth box.")
  ...
  private fun importPrefs(source: LegacyMigrationSource): LegacyMigrationStepResult {
    return runCatching {
        ...
        when (report.status) {
          ...
          LegacyMigrationPhaseStatus.PartiallyImported -> LegacyMigrationStepResult.Imported

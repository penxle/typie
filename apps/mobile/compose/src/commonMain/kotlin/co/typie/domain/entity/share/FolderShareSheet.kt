...
private val FolderShare_entity.folder
  get() = requireNotNull(node.onFolder)

private fun resolveFolderShareTitle(count: Int): String {
  return if (count <= 1) "이 폴더 공유하기" else "폴더 ${count}개 공유하기"
}
...
internal fun FolderShareSheet(
  ...
  val initialThumbnailUrl = folders.firstOrNull()?.folder?.thumbnail?.url
  val initiallyMixedThumbnail =
    remember(folders) { hasMixedValues(folders.map { it.folder.thumbnail?.id }) }
  val form =
    remember(folderIds) {
      FolderShareForm(
        scope,

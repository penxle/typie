...
class EntityContainerHelpersTest {
  @Test
  fun `calculateEntityReorderOrdersFromOrderedKeys returns upper order when moved to first position`() {
    ...
    assertEquals(EntityReorderOrders(lowerOrder = null, upperOrder = "10"), result)
  }
  ...
  fun `calculateEntityReorderOrdersFromOrderedKeys returns adjacent orders when moved to middle position`() {
    ...
    assertEquals(EntityReorderOrders(lowerOrder = "10", upperOrder = "20"), result)
  }
  ...
  fun `calculateEntityReorderOrdersFromOrderedKeys returns lower order when moved to last position`() {
    ...
    assertEquals(EntityReorderOrders(lowerOrder = "30", upperOrder = null), result)
  }
  ...
  fun `calculateEntityReorderOrdersFromOrderedKeys returns null when moved key is missing`() {
    val result =
      calculateEntityReorderOrdersFromOrderedKeys(
        items = listOf(orderedItem(id = "a", order = "10"), orderedItem(id = "b", order = "20")),
  ...
  @Test
  fun `displayEntityRows returns ordered items when keys cover the full set`() {
    val result =
      displayEntityRows(
        items =
          listOf(
  ...
  @Test
  fun `displayEntityRows falls back to server order when ordered keys are incomplete`() {
    val result =
      displayEntityRows(
        items =
          listOf(

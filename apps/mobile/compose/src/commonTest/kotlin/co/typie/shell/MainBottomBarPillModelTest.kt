...
class MainBottomBarPillModelTest {
  ...
  fun `main tabs expose home space and notes`() {
    assertEquals(listOf(Tab.Home, Tab.Studio, Tab.Notes), Tab.entries)
  }

  @Test
  fun cumulativeCenters_returnsHalfWidthForFirstAndOffsetForRest() {
    val widths = mapOf(Tab.Home to 120f, Tab.Studio to 64f, Tab.Notes to 64f)
    val centers = cumulativeCenters(widths)

    assertEquals(60f, centers.getValue(Tab.Home))
    assertEquals(152f, centers.getValue(Tab.Studio))
    assertEquals(216f, centers.getValue(Tab.Notes))
  }

  @Test
  fun cumulativeCenters_handlesSwappedActiveWidths() {
    val widths = mapOf(Tab.Home to 64f, Tab.Studio to 120f, Tab.Notes to 64f)
    val centers = cumulativeCenters(widths)

    assertEquals(32f, centers.getValue(Tab.Home))
    assertEquals(124f, centers.getValue(Tab.Studio))
    assertEquals(216f, centers.getValue(Tab.Notes))
  }

  @Test
  fun nearestTab_picksClosestCenterToPointer() {
    val centers = mapOf(Tab.Home to 60f, Tab.Studio to 152f, Tab.Notes to 216f)
    ...
    assertEquals(Tab.Studio, nearestTab(centers, totalWidth = 248f, pointerX = 107f))
    assertEquals(Tab.Studio, nearestTab(centers, totalWidth = 248f, pointerX = 184f))

...
class AndroidLegacyMigrationPlatformSource(
  private val context: Context,
) : LegacyMigrationPlatformSource {
  override suspend fun load(): LegacyMigrationSource? = withContext(Dispatchers.IO) {
    ...
    val authBoxSource = if (authBoxBytes != null && base64HiveKey != null) {
      val keyCrc = calculateLegacyHiveKeyCrc(base64HiveKey) ?: return@withContext nullSource(
  ...
  private fun findLegacyHiveDirectory(): File? {
    ...
    return candidates.firstOrNull { directory ->
      directory.exists() && LEGACY_HIVE_BOX_NAMES.any { boxName -> File(directory, "$boxName.hive").exists() }
    }
  }
  ...
  private fun readHiveEncryptionKey(): String? {
    ...
    return attempts.firstNotNullOfOrNull { algorithms ->
      runCatching {
        decryptFlutterSecureStorageValue(
  ...
  private fun decryptFlutterSecureStorageValue(
    ...
    val secretKey = unwrapStorageKey(
      wrappedKey = wrappedKey,
      keyAlgorithm = algorithms.key,
    )
    val decryptedBytes = algorithms.storage.decrypt(
  ...
  private fun unwrapStorageKey(
    ...
    val privateKey = keyStore.getKey(keyAlgorithm.alias(context.packageName), null) as? PrivateKey
  ...
  private fun calculateLegacyHiveKeyCrc(base64HiveKey: String): Long? {
    val hiveKey = runCatching { Base64.decode(base64HiveKey, Base64.DEFAULT) }.getOrNull() ?: return null
    val digest = MessageDigest.getInstance("SHA-256").digest(hiveKey)
    val crc32 = CRC32().apply { update(digest) }
    return crc32.value
  }

  private fun decryptLegacyHivePayload(
    payload: ByteArray,
    base64HiveKey: String,
  ): ByteArray? {
    val hiveKey = runCatching { Base64.decode(base64HiveKey, Base64.DEFAULT) }.getOrNull() ?: return null
  ...
  private fun nullSource(
    preferenceBox: ByteArray?,
    themeBox: ByteArray?,
  ): LegacyMigrationSource? {
  ...
  private data class AndroidLegacySecureStorageAlgorithms(
    ...
    companion object {
      val LegacyDefault = AndroidLegacySecureStorageAlgorithms(
        key = AndroidLegacyKeyCipherAlgorithm.RsaPkcs1,
        storage = AndroidLegacyStorageCipherAlgorithm.AesCbc,
      )
      val CurrentDefault = AndroidLegacySecureStorageAlgorithms(
      ...
      fun fromConfig(context: Context): AndroidLegacySecureStorageAlgorithms {
        val preferences = context.getSharedPreferences(FLUTTER_SECURE_STORAGE_CONFIG_PREFS_NAME, Context.MODE_PRIVATE)
        val key = when (preferences.getString(FLUTTER_SECURE_STORAGE_CONFIG_KEY_ALGORITHM, null)) {
          AndroidLegacyKeyCipherAlgorithm.RsaOaep.rawValue -> AndroidLegacyKeyCipherAlgorithm.RsaOaep
          else -> AndroidLegacyKeyCipherAlgorithm.RsaPkcs1
        }
        val storage = when (preferences.getString(FLUTTER_SECURE_STORAGE_CONFIG_STORAGE_ALGORITHM, null)) {
          AndroidLegacyStorageCipherAlgorithm.AesGcm.rawValue -> AndroidLegacyStorageCipherAlgorithm.AesGcm
  ...
  private enum class AndroidLegacyKeyCipherAlgorithm(
    val rawValue: String,
  ) {
    ...
    fun createCipher(): Cipher {
      ...
      return runCatching { Cipher.getInstance(transformation, ANDROID_KEYSTORE_WORKAROUND_PROVIDER) }
...
private const val FLUTTER_SECURE_STORAGE_KEY_PREFIX = "VGhpcyBpcyB0aGUgcHJlZml4IGZvciBhIHNlY3VyZSBzdG9yYWdlCg"

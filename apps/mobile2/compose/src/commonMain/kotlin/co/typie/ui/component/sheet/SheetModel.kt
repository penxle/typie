...
sealed interface SheetResult<out R> {
  data class Completed<R>(val value: R) : SheetResult<R>

  data class Dismissed(val reason: SheetDismissReason) : SheetResult<Nothing>
}
...
sealed interface SheetDetentId {
  ...
  @Immutable data class Fixed(val height: Dp) : SheetDetentId

  @Immutable data class Fraction(val fraction: Float) : SheetDetentId

  @Immutable data class TopGap(val gap: Dp) : SheetDetentId

  @Immutable data class Content(val maxTopGap: Dp?) : SheetDetentId

  @Immutable data class Custom(val value: String) : SheetDetentId
}

@Immutable data class SheetDetentContext(val viewportHeight: Dp, val contentHeight: Dp)
...
sealed interface SheetDetent {
  ...
  @Immutable
  data class Fixed(val height: Dp) : SheetDetent {
  ...
  @Immutable
  data class TopGap(val gap: Dp) : SheetDetent {
    override val id: SheetDetentId = SheetDetentId.TopGap(gap)
  }

  @Immutable
  data class Content(val maxTopGap: Dp? = null) : SheetDetent {
    override val id: SheetDetentId = SheetDetentId.Content(maxTopGap)
  }

  class Custom(override val id: SheetDetentId, val resolver: (SheetDetentContext) -> Dp) :
    SheetDetent
}

@Immutable data class ResolvedSheetDetent(val id: SheetDetentId, val height: Dp)

sealed interface SheetSizePolicy {
  @Immutable data class Intrinsic(val topGap: Dp = SheetDefaults.IntrinsicTopGap) : SheetSizePolicy
...
data class SheetOverlaySpec(
  ...
  init {
    require(mode != SheetMode.Persistent) { "SheetOverlaySpec only supports overlay modes." }
  }
}
...
internal fun SheetSizePolicy.requiresContentMeasurement(): Boolean =
  when (this) {
    ...
    is SheetSizePolicy.Detents ->
      (listOf(initial) + available).any { it.requiresContentMeasurement() }
  }
...
internal fun SheetDetent.requiresContentMeasurement(): Boolean =
  when (this) {
    ...
    is SheetDetent.Custom -> true

    is SheetDetent.Fixed,
    is SheetDetent.Fraction,
    is SheetDetent.TopGap -> false
  }

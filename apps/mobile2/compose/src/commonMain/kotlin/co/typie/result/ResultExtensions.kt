...
inline fun <T, E> Result<T, E>.onOk(block: (T) -> Unit): Result<T, E> = also {
  if (this is Result.Ok) block(value)
}

inline fun <T, E> Result<T, E>.onErr(block: (E) -> Unit): Result<T, E> = also {
  if (this is Result.Err) block(error)
}

inline fun <T, E> Result<T, E>.onException(block: (Throwable) -> Unit): Result<T, E> = also {
  if (this is Result.Exception) block(exception)
}

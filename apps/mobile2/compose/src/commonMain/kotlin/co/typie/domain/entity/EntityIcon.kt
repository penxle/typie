...
// Keep this in sync with the website picker so mobile app renders the same entity icon names.
val EntityIcons =
  mapOf(
    "file" to Lucide.File,
...
data class EntityIconColor(val label: String, val color: Color)

@get:Composable
val EntityIconColors
  get() =
    mapOf(
      "gray" to EntityIconColor("그레이", AppTheme.colors.palette.gray),
      "red" to EntityIconColor("레드", AppTheme.colors.palette.red),
      "orange" to EntityIconColor("오렌지", AppTheme.colors.palette.orange),
      "yellow" to EntityIconColor("옐로", AppTheme.colors.palette.yellow),
      "green" to EntityIconColor("그린", AppTheme.colors.palette.green),
      "blue" to EntityIconColor("블루", AppTheme.colors.palette.blue),
      "purple" to EntityIconColor("퍼플", AppTheme.colors.palette.purple),
    )
...
fun EntityIcon(entity: EntityIcon_entity, modifier: Modifier = Modifier) {
  val fallbackIcon =
    when (entity.type) {
      EntityType.FOLDER -> Lucide.Folder
      else -> Lucide.File
    }

  val icon = EntityIcons[entity.icon] ?: fallbackIcon
  val color = EntityIconColors[entity.iconColor]?.color ?: AppTheme.colors.palette.gray

  Icon(icon = icon, tint = color, modifier = modifier)
}

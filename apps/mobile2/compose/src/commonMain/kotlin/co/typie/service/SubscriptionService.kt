...
data class SubscriptionCelebration(val title: String, val message: String)
...
object SubscriptionService {
  ...
  suspend fun purchase(product: PurchaseProduct, accountId: String): PurchaseStartResult {
    if (usesSandbox) {
      SubscriptionDevSandbox.purchase(product.interval)

      return PurchaseStartResult(started = true, celebration = purchaseCelebration())
    }

    return PurchaseStartResult(
      started = PlatformModule.purchaseService.purchase(product = product, accountId = accountId)
    )
  }
...
internal fun trialCelebration(): SubscriptionCelebration {
  return SubscriptionCelebration(title = "무료 체험이 시작됐어요!", message = "2주간 타이피의 모든 기능을 자유롭게 이용해보세요.")
}

internal fun purchaseCelebration(): SubscriptionCelebration {
  return SubscriptionCelebration(title = "구독이 시작됐어요!", message = "타이피의 모든 기능을 자유롭게 이용해보세요.")
}

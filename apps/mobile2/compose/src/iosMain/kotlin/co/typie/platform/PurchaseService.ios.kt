...
internal class IOSPurchaseService : PurchaseService {
    ...
    return withContext(Dispatchers.Main) {
      suspendCancellableCoroutine { continuation ->
        bridge.queryProductsWithCompletion { payloads, error ->
          ...
          val products = payloads
            .orEmpty()
            .filterIsInstance<PurchaseProductPayload>()
            .mapNotNull { payload -> payload.toPurchaseProduct() }
  ...
  override suspend fun purchase(
    ...
    return withContext(Dispatchers.Main) {
      suspendCancellableCoroutine { continuation ->
        bridge.purchaseProductWithProductId(
          productId = product.id,
          accountId = accountId,
        ) { success: Boolean, _: Any? ->
  ...
  override suspend fun openSubscriptionManagement(): Boolean {
    ...
    return withContext(Dispatchers.Main) {
      suspendCancellableCoroutine { continuation ->
        bridge.openSubscriptionManagementWithCompletion { success ->
          continuation.resume(success)
        }
      }
    }
...
private fun PurchaseEventPayload.toPurchaseEvent(): PurchaseEvent {
  return when (type) {
    "restored" -> PurchaseEvent.Restored(
      productId = productId,

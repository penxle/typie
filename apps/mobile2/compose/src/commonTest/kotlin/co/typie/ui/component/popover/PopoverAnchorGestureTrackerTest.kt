...
class PopoverAnchorGestureTrackerTest {
  ...
  fun consumesGesture_fromPressUntilRelease() {
    val tracker = PopoverAnchorGestureTracker(
      origin = Offset(24f, 32f),
      armDistancePx = 9f,
    )
    ...
    val moved = tracker.update(
      currentPosition = Offset(26f, 34f),
      elapsedMillis = 50L,
      isPressed = true,
    )
  ...
  fun doesNotArm_beforeDelayOrDistance() {
    ...
    val beforeDelay = tracker.update(
    ...
    val belowDistanceTracker = PopoverAnchorGestureTracker(
      origin = Offset(10f, 10f),
      armDistancePx = 9f,
    )
    belowDistanceTracker.start()
    val belowDistance = belowDistanceTracker.update(

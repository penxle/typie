...
class SheetDetentResolverTest {
  ...
  fun detentPolicyResolvesAndSortsDistinctHeights() {
    val resolved = SheetDetentResolver.resolve(
      ...
      context = SheetDetentContext(
        viewportHeight = 900.dp,
        contentHeight = 700.dp,
      ),
    )

    assertEquals(
      listOf(280.dp, 450.dp, 820.dp),
      resolved.map { it.height },
    )
    assertEquals(
      listOf(
  ...
  fun fixedAndTopGapDetentsDoNotRequireContentMeasurement() {
    val policy = SheetSizePolicy.Detents(
      initial = SheetDetent.Fixed(360.dp),
      available = listOf(
        SheetDetent.Fixed(360.dp),
        SheetDetent.TopGap(64.dp),
      ),
    )
  ...
  fun dragDismissDefaultsToMinDetentAnchor() {
    ...
    val shouldDismiss = shouldDismissDraggedSheet(
      policy = SheetSizePolicy.Detents(
        initial = SheetDetent.Fixed(360.dp),
        available = listOf(
          SheetDetent.Fixed(360.dp),

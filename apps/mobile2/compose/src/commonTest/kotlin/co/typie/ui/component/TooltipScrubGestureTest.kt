...
class TooltipScrubGestureTest {
  ...
  @Test
  fun resolveTooltipTapGestureAction_treatsConsumedReleaseWithinToleranceAsTap() {
    val result =
      resolveTooltipTapGestureAction(isPressed = false, isConsumed = true, movementDistancePx = 0f)

    assertEquals(TooltipTapGestureAction.Tap, result)
  }

  @Test
  fun resolveTooltipTapGestureAction_cancelsReleaseWhenMovementExceedsTolerance() {
    val result =
      resolveTooltipTapGestureAction(
        isPressed = false,
        isConsumed = false,
        movementDistancePx = TapGestureMovementTolerancePx + 1f,
      )

    assertEquals(TooltipTapGestureAction.Cancel, result)
  }

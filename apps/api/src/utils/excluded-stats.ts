...
export const getExcludedDeltasByDocument = async (params: {
  userId: string;
  from: Dayjs;
  to: Dayjs;
}): Promise<Map<string, { additions: number; deletions: number }>> => {
  const rows = await db
    .select({
      documentId: DocumentHeads.documentId,
      additions: sql`COALESCE(SUM(${DocumentHeadContributors.additions}), 0)`.mapWith(Number),
      deletions: sql`COALESCE(SUM(${DocumentHeadContributors.deletions}), 0)`.mapWith(Number),
    })
    .from(DocumentHeads)
    ...
    .groupBy(DocumentHeads.documentId);

  return new Map(rows.map((row) => [row.documentId, { additions: row.additions, deletions: row.deletions }]));
};

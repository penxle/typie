...
export const PRISM_DEVICE_ID = 'UDEV0PRISM0000000';
...
export const ensurePrismActor = async (): Promise<void> => {
  const missing = new Error('prism actor missing — run the seed migration first');

  await db.select({ id: Users.id }).from(Users).where(eq(Users.id, PRISM_USER_ID)).then(firstOrThrowWith(missing));
  await db.select({ id: UserDevices.id }).from(UserDevices).where(eq(UserDevices.id, PRISM_DEVICE_ID)).then(firstOrThrowWith(missing));
};

...
import { calculatePaymentAmount, makePeriodPayment } from '@/utils';
...
builder.mutationFields((t) => ({
  ...
  enrollPlan: t.withAuth({ session: true }).fieldWithInput({
    ...
    resolve: async (_, { input }, ctx) => {
      ...
      const { userPlan, periodPaymentResult } = await db.transaction(async (tx) => {
        ...
        const periodPaymentResult = await makePeriodPayment({
          tx,
          invoiceId: invoice.id,
        });

        return { userPlan, periodPaymentResult };
      });

      if (periodPaymentResult?.makePaymentResult) {
        // 이거 실패했다고 에러 내보내는건 혼동을 주는듯

        await db
          .update(PaymentRecords)
          .set({
            receiptUrl: periodPaymentResult.makePaymentResult.receiptUrl,
          })
          .where(eq(PaymentRecords.id, periodPaymentResult.paymentRecordId))

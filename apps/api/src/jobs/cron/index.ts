...
const key = Bun.hash.xxHash32(dev ? `jobs:cron:${nanoid()}` : `jobs:cron:${env.PUBLIC_PULUMI_STACK}`);

const tryAcquireLock = async () => {
  const res = await db.execute<{ locked: boolean }>(sql`SELECT pg_try_advisory_lock(${key}) as locked -- @silent@`).then(first);

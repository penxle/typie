...
import { db, Documents, DocumentStates, Entities, firstOrThrow, Folders } from '#/db/index.ts';
...
export const DocumentIndexJob = defineJob('search:index:document', async (documentId: string) => {
  const document = await db
    ...
    .from(Documents)
    .innerJoin(DocumentStates, eq(Documents.id, DocumentStates.documentId))

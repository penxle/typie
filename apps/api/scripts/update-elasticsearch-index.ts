...
import { db, Documents, DocumentStates, Entities, Folders } from '#/db/index.ts';
    ...
    const documents = await db
      ...
      .where(
        and(
          ...
          eq(DocumentStates.projectionDegraded, false),
          cursor ? sql`${Documents.id} > ${cursor}` : undefined,
        ),
      )
      ...
      .where(
        and(
          or(
            ...
            and(eq(Entities.state, EntityState.ACTIVE), eq(DocumentStates.projectionDegraded, true)),
          ),
          cursor ? sql`${Documents.id} > ${cursor}` : undefined,
        ),
      )
...
try {
  ...
  const [activeDocCount, deletedDocCount, activeFolderCount, deletedFolderCount] = await Promise.all([
    db
      ...
      .innerJoin(Entities, eq(Entities.id, Documents.entityId))
      .innerJoin(DocumentStates, eq(DocumentStates.documentId, Documents.id))
      .where(and(eq(Entities.state, EntityState.ACTIVE), eq(DocumentStates.projectionDegraded, false)))
      .then((r) => r[0].count),
    db
      .select({ count: count() })
      .from(Documents)

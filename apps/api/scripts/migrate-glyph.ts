...
const migrateNode = async ({ node, userId, tx }: MigrateNodeParams): Promise<JSONContent> => {
  ...
  const newNode = await match(node.type)
    ...
    .with('file', async () => {
      ...
      const blob = await ky(`https://glyph.pub/${originalFile.path}`).blob();
    ...
    .with('image', async () => {
      ...
      const arrayBuffer = await ky(`https://glyph.pub/${originalImage.path}`).arrayBuffer();
    ...
    .with('gallery', async () => {
      ...
      return await Promise.all(
        ids.map(async (id) => {
          ...
          const blob = await ky(`https://glyph.pub/${originalImage.path}`).blob();
          const arrayBuffer = await blob.arrayBuffer();
...
const migratePost = async (params: MigratePostParams) => {
  return await db.transaction(async (tx) => {
    const body = await migrateNode({ node: params.body, userId: params.userId, tx });
...
while (true) {
  ...
  const user = await db.select().from(Users).where(eq(Users.email, typieEmail)).then(firstOrThrow);
  const site = await db.select().from(Sites).where(eq(Sites.userId, user.id)).then(firstOrThrow);
  let lastFolderOrder = await db
  ...
  for (const glyphSpace of glyphSpaces) {
    const parentEntity = await db.transaction(async (tx) => {
      const entity = await tx
        .insert(Entities)
        .values({
          userId: user.id,

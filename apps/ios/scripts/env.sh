...
keys='["API_URL","AUTH_URL","WS_URL","OIDC_CLIENT_ID","OIDC_CLIENT_SECRET","GOOGLE_IOS_CLIENT_ID","GOOGLE_DOT_REVERSED_IOS_CLIENT_ID","GOOGLE_SERVER_CLIENT_ID","KAKAO_NATIVE_APP_KEY","NAVER_CLIENT_ID","NAVER_CLIENT_SECRET"]'
config_tmp=$(mktemp)
...
printf '%s' "$downloaded" | jq -r --argjson keys "$keys" '
    ...
    | if ($key == "WS_URL") and (test("^wss?://[^/]+") | not) then error("invalid URL in \($key)") else . end
    | "\($key) = \(gsub("//"; "/$()/"))"
  ' > "$config_tmp"

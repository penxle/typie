...
final class MainTabBarController: UITabBarController, UITabBarControllerDelegate {
  ...
  private func installCustomBar() -> UIButton {
    ...
    let menuHost = ThemedHostingController.make(title: "", MoreMenu(state: menuState))
    addChild(menuHost)
    let bar = MainTabBar(
    ...
    bar.onSelect = { [weak self] tab in
      ...
      selectedTab = tabs[MainTab.allCases.firstIndex(of: tab) ?? 0]
    ...
    NSLayoutConstraint.activate([
      ...
      bar.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -MainTabBar.bottomPadding),
    ])
...
extension MainTab {
  ...
  var image: UIImage? {
    ...
    return UIImage(named: name.assetName, in: DesignBundle.bundle, with: nil)
  }

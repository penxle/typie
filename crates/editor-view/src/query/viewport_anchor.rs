...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub struct CapturedViewportAnchor {
    pub identity: ViewportAnchor,
    pub geometry: ResolvedViewportAnchor,
}
...
impl ViewportAnchorPresentation {
    ...
    pub fn capture_selection_head(&self) -> Option<CapturedViewportAnchor> {
        capture_selection_head(&self.state, &self.layout_index)
    }

    pub fn capture_page_point(&self, point: ViewportAnchorPoint) -> Option<CapturedViewportAnchor> {
...
fn capture(
    ...
    let ViewportAnchorResolution::Resolved { geometry } = resolve(state, layout_index, &identity)
    else {
        return None;
    };
    Some(CapturedViewportAnchor { identity, geometry })
}
...
fn resolve_node(
    ...
    let Some(entry) = layout_index.entry_for_content_node(&node) else {
        return ViewportAnchorResolution::NotLaidOut;
    };
...
mod tests {
    ...
    #[test]
    fn live_identity_without_layout_geometry_is_not_laid_out() {
        let mut projected = ProjectedState::empty();
        projected.commit();
        let before = State::new(projected.clone(), None);
        let mut view = View::new_test();
        view.layout(&before);
        let presentation = view.capture_viewport_anchor_presentation().unwrap();
        ...
        let image = projected
            ...
            .unwrap()
            .id;
        let after = State::new(projected, None);
        let identity = super::ViewportAnchor::Node {
            node: image,
            offset_x: 0.0,
            offset_y: 0.0,
        };
        ...
        assert_eq!(
            super::resolve(&after, &presentation.layout_index, &identity),
            super::ViewportAnchorResolution::NotLaidOut,
        );
    }
}

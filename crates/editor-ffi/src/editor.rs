...
impl Editor {
    ...
    pub fn retain_viewport_anchor_presentation(
        &self,
        revision: Complex<editor_core::Revision>,
    ) -> EditorResult<bool> {
        let revision = revision.from_ffi()?;
        self.with_inner(|inner| Ok(inner.editor.retain_viewport_anchor_presentation(revision)))
    }

    pub fn release_viewport_anchor_presentation(
        &self,
        revision: Complex<editor_core::Revision>,
    ) -> EditorResult<()> {
        let revision = revision.from_ffi()?;
        self.with_inner(|inner| {
            inner.editor.release_viewport_anchor_presentation(revision);
            Ok(())
        })
    }
    ...
    pub fn capture_selection_viewport_anchor(
        &self,
        revision: Complex<editor_core::Revision>,
    ) -> EditorResult<Option<Complex<editor_view::ViewportAnchor>>> {
        let revision = revision.from_ffi()?;
        self.with_inner(|inner| {
            Ok(inner
                .editor
...
mod tests {
    ...
    fn viewport_anchor_contract_round_trips_through_ffi() {
        ...
        assert!(
            editor
                .retain_viewport_anchor_presentation(revision)
                .unwrap()
        );
        let anchor = editor
            .capture_selection_viewport_anchor(revision)
        ...
        let resolution = editor.resolve_viewport_anchor(revision, anchor).unwrap();
        ...
        editor
            .release_viewport_anchor_presentation(revision)
            .unwrap();
    }

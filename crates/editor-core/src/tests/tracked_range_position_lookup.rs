...
fn containing_position_is_start_inclusive_and_end_exclusive() {
    ...
    let at_start = editor.tracked_ranges_containing_position(Position::new(t1, 1), Some("comment"));
    assert_eq!(
        at_start.iter().map(|r| r.id.as_str()).collect::<Vec<_>>(),
        ["r"]
    );

    let at_end = editor.tracked_ranges_containing_position(Position::new(t1, 4), Some("comment"));
    assert!(
        at_end.is_empty(),
...
fn containing_position_filters_group_and_sorts_narrowest_first() {
    ...
    editor.apply(add_msg(
        "inner",
        "comment-active",
        Selection::new(Position::new(t1, 1), Position::new(t1, 4)),
    ));
    editor.apply(add_msg(
        "spellcheck",
        "spellcheck",
    ...
    let all = editor.tracked_ranges_containing_position(Position::new(t1, 2), None);
    assert_eq!(
    ...
    let comments = editor.tracked_ranges_containing_position(Position::new(t1, 2), Some("comment"));
    assert_eq!(

...
mod tests {
    ...
    fn retype_into_empty_paragraph_restores_marker_style() {
        ...
        let retyped_styled = {
            ...
            view.node(p1)
                .map(|p| {
                    p.children().any(|c| match c {
                        ChildView::Leaf(l) => {
                            actual.projected.node_styles().value_of(l.dot())
                                == Some("s1".to_string())
                        }
                        _ => false,
                    })
        ...
        assert!(retyped_styled, "retyped run gets marker style");
        let marker_after = actual.projected.node_markers().value_of(p1);
        assert_eq!(marker_after, None, "marker cleared from paragraph");
    }
    ...
    fn insert_with_pending_style_stamps_style_on_new_run() {
        ...
        let (initial, para, ..) = state! {
            doc {
                styles { s1: "s" [] }
                root { para: paragraph { text("Hello") } }
            }
            selection: (para, 5)
        };

...
use editor_state::{enclosing_table, selected_table_cell_ids};
...
pub fn set_table_cell_background_color(
    ...
    let cell_ids: Vec<Dot> = {
        ...
        let cell_ids = selected_table_cell_ids(&view, &sel, &resolved).ok_or_else(|| {
            CommandError::Corrupted("selection is not a cell background target".into())
        })?;
        if cell_ids.is_empty()
            || cell_ids
                .iter()
                .any(|cell| enclosing_table(&view, *cell) != Some(table_id))
        {
...
mod tests {
    ...
    use editor_state::{Position, Selection, cell_rect_selection};
    ...
    fn sets_background_on_selected_cells() {
        ...
        let (actual, ..) = transact!(initial, |tr| set_table_cell_background_color(
            &mut tr,
            tbl,
            Some("red".to_string())
        ));
    ...
    #[test]
    fn collapsed_selection_in_table_cell_sets_current_cell_background_without_changing_selection() {
        ...
        let initial_selection = initial.selection;
        let (actual, ..) = transact!(initial, |tr| set_table_cell_background_color(
            &mut tr,
            tbl,
            Some("red".to_string()),
        ...
        let view = actual.view();
        assert_eq!(actual.selection, initial_selection);
        assert_eq!(cell_bg(&view, r0c0), Some("red".to_string()));
        assert_eq!(cell_bg(&view, r0c1), None);
    }

    #[test]
    fn range_selection_inside_table_cell_sets_current_cell_background_without_changing_selection() {
        let (initial, tbl, r0c0, para, r0c1, ..) = state! {
            doc { root {
                tbl: table {
                    table_row {
                        r0c0: table_cell { para: paragraph { text("AB") } }
                        r0c1: table_cell { paragraph { text("C") } }
                    }
                }
            } }
            selection: (r0c0, 0)
    ...
    #[test]
    fn table_block_selection_sets_all_cell_backgrounds_without_changing_selection() {
        let (initial, tbl, r0c0, r0c1, r1c0, r1c1, ..) = state! {
            doc { root {
                tbl: table {
                    table_row {
        ...
        let initial = editor_state::State {
            selection: Some(Selection::new(
                Position::new(editor_crdt::Dot::ROOT, 0),
                Position::new(editor_crdt::Dot::ROOT, 1),
            )),
            ..initial
        };

...
use crate::helpers::{
    find_ancestor_textblock, resolve_effective_modifiers, resolve_inherited_modifiers,
};
...
pub fn toggle_bold(tr: &mut Transaction, resource: &Resource) -> CommandResult {
    ...
    // Compact affected textblocks: merge adjacent text nodes with identical modifiers
    let doc = tr.doc();
...
fn collect_text_nodes_in_range(
    tr: &mut Transaction,
    from: &Position,
    to: &Position,

    ...
    for child in ctx.node.children() {
        if let Node::Text(node) = child.node() {
            ...
            for segment in segments {
                ...
                for annotation_id in &segment.annotations {
                    if let Some(Annotation::Ruby(ruby_ann)) =
                        ctx.node.get_annotation(*annotation_id)
                    {
                ...
                match child.node() {
                    Node::Text(node) => {
                        ...
                        for segment in segments {
                            ...
                            for annotation_id in &segment.annotations {
                                if let Some(annotation) = ctx.node.get_annotation(*annotation_id) {
                                    let (start, end) = map_range_with_preedit(
                                        (base_start, base_end),
                                        preedit_info,
                                        &Expand::None,

use crate::model::{Annotation, Codec, Node, NodeId};
...
pub fn snapshot_to_json(snapshot: &[u8]) -> Result<DocumentJson> {
    ...
    let annotations_map = loro.get_map("annotations");
    let mut annotations = HashMap::new();
    if let LoroValue::Map(entries) = annotations_map.get_deep_value() {
        for (key, _) in entries.iter() {
            if let Some(ann_map) = annotations_map
                .get(key)
                .and_then(|v| v.into_container().ok())
                .and_then(|c| c.into_map().ok())
            {
                if let Some(ann) = Annotation::decode_from_map(&ann_map) {
...
pub fn json_to_snapshot(doc_json: &DocumentJson) -> Result<Vec<u8>> {
    ...
    if !doc_json.annotations.is_empty() {
        let annotations_map = loro.get_map("annotations");
        for (key, ann) in &doc_json.annotations {
            let ann_map = annotations_map.insert_container(key, LoroMap::new())?;
            ann.encode_to_map(&ann_map)?;
        }
    }

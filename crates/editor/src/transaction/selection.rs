...
use crate::utils::compute_word_boundaries;
...
impl Transaction {
    pub fn select_word_at(&mut self, position: Position) -> Result<bool> {
        ...
        if let Some((child_id, local_offset)) = find_child_at_offset(&block, position.offset) {
            ...
            let anchor = Position::new(position.node_id, 0, Affinity::Downstream);

            let node = block;
            let next_id = node.next_sibling().map(|n| n.node_id());

            let head = if let Some(next_id) = next_id {
                let next = self.node(next_id).context("Node not found")?;
                if let Some(first_child) = next.first_child() {
                    if matches!(first_child.node(), Node::Text(_)) {
                        Position::new(next.node_id(), 0, Affinity::Downstream)
                    } else {
                        anchor
                    }
                } else if next.spec().content.matches(NodeType::Text) {
                    Position::new(next.node_id(), 0, Affinity::Downstream)
                } else {
                    anchor
                }
            ...
            if anchor == head && node.first_child().is_some() {
                return Ok(false);
            }

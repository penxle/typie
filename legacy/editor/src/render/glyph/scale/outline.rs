...
impl Outline {
    ...
    pub fn get(&self, index: usize) -> Option<Layer<'_>> {
    ...
    pub fn get_mut(&mut self, index: usize) -> Option<LayerMut<'_>> {
        let data = self.layers.get(index)?;
...
impl Outline {
    ...
    pub fn begin_layer(&mut self, color_index: Option<u16>) {
...
/// Compute winding for a multi-contour outline by summing per-contour
/// signed areas. This avoids spurious edges between contours that would
/// corrupt the global area calculation.
...
/// Compute winding for a single contour (used by the embolden free function).
fn compute_winding(points: &[Point]) -> u8 {
    if contour_area(points) > 0. { 1 } else { 0 }
}
...
/// Adapter that implements skrifa's OutlinePen to write into an Outline.
